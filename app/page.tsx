...
function chartPeriodLabel(period: string): string {
  const quarter = period.match(/^(\d{4})-(03|06|09|12)-\d{2}$/);
  if (quarter) return `${quarter[1]} Q${({ "03": 1, "06": 2, "09": 3, "12": 4 } as Record<string, number>)[quarter[2]]}`;
  const month = period.match(/^(\d{4})-(\d{2})/);
  return month ? `${month[1]}-${month[2]}` : period;
}
...
function TrendChart({ factor }: { factor: Factor }) {
  ...
  return <figure className="trend-chart" aria-label={`${factor.name}數據變化圖`}>
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${factor.name}數據變化`}>
      ...
      {points.map((point, index) => <g key={`${point.label}-${point.value}`}><circle cx={toX(index)} cy={toY(point.value)} r="4" />{(index === 0 || index === points.length - 1 || index % labelInterval === 0) ? <text x={toX(index)} y={height - 4} textAnchor="middle">{chartPeriodLabel(point.label)}</text> : null}</g>)}
      {points.length === 0 ? <text className="chart-empty" x={width / 2} y={height / 2} textAnchor="middle">尚無可繪製的歷史數值</text> : null}
    </svg>
    <figcaption>{points.length ? `共 ${points.length} 個資料期；最新：${points.at(-1)?.display ?? "資料待補"}` : "尚無可繪製的歷史數值"}</figcaption>
  </figure>;
}
...
export default function Home() {
  ...
  const requiresFinancialModel = selected?.market === "台股" && selected.industry === "17";
