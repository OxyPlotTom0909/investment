...
export default function Home() {
  ...
  useEffect(() => {
    void fetch("/api/market").then(async (response) => {
      ...
      setSnapshot(data);
      setSelectedKey(data.companies[0] ? `${data.companies[0].market}:${data.companies[0].ticker}` : null);
    }).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "市場資料讀取失敗"));
  }, []);

  const companies = useMemo(() => (snapshot?.companies ?? []).filter((company) => (market === "全部" || company.market === market) && (grade === "全部" || company.grade === grade)), [snapshot, market, grade]);
  const selected = companies.find((company) => `${company.market}:${company.ticker}` === selectedKey) ?? companies[0] ?? null;
  ...
  return <main>
    ...
    <section className="controls" aria-label="篩選條件"><div><label htmlFor="market">市場</label><select id="market" value={market} onChange={(event) => setMarket(event.target.value as Market)}><option>全部</option><option>台股</option><option>美股</option></select></div><div><label htmlFor="grade">品質級別</label><select id="grade" value={grade} onChange={(event) => setGrade(event.target.value as GradeFilter)}><option>全部</option><option>A</option><option>B</option><option>C</option><option>資料不足</option></select></div><aside className="rule-card"><strong>分級規則</strong><span>11 項全數可評後：A ≥ 8、B 5–7、C ≤ 4</span></aside></section>
