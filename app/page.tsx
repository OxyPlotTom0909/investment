...
export default function Home() {
  ...
  useEffect(() => { void fetch("/api/market").then(async (response) => { if (!response.ok) { const body = await response.json() as { error?: string }; throw new Error(body.error ?? "市場資料讀取失敗"); } return response.json() as Promise<MarketSnapshot>; }).then(setSnapshot).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : "市場資料讀取失敗")); }, []);
  ...
  const selectCompany = (company: Company): void => setSelectedKey(companyKey(company));
  ...
  return <main>
    ...
    {snapshot && !selected ? <section className="company-overview" aria-label="全部公司清單"><div className="overview-top list-card"><div className="section-heading"><span className="eyebrow">{hasActiveFilter ? `符合條件的公司（${companies.length} 家）` : `全部公司（${companies.length} 家）`}</span></div>{companies.length === 0 ? <p className="empty-state">{isUnratedUsSelection ? "美股尚未完成品質評分，因此目前沒有 A／B／C 級結果；可選擇「資料不足」查看已收錄名單。" : "沒有符合目前篩選條件的公司。"}</p> : <div className="company-list">{companies.map((company) => <CompanyRow company={company} key={companyKey(company)} onSelect={selectCompany} />)}</div>}</div><div className="overview-bottom"><div className="section-heading"><div><span className="eyebrow">已評合格</span><h2>A、B 級公司</h2></div><span className="qualified-count">{qualifiedCompanies.length} 家</span></div>{qualifiedCompanies.length === 0 ? <p className="empty-state">目前篩選條件下沒有 A 或 B 級公司。</p> : <div className="qualified-grid">{qualifiedCompanies.map((company) => <button className="qualified-card" key={companyKey(company)} onClick={() => selectCompany(company)}><span className={`grade ${gradeClass(company.grade)}`}>{company.grade}</span><div><strong>{company.name}</strong><small>{company.ticker} · {company.market}</small><span>{company.passedCount} / {company.evaluatedCount} 項符合</span></div></button>)}</div>}</div></section> : null}
    {snapshot && selected ? <article className="detail-card detail-view"><div className="detail-top"><div><button className="back-button" onClick={() => setSelectedKey(null)}>← 回全部公司</button><span className="eyebrow">{selected.market} · {selected.industry}</span><h2>{selected.name} <span>{selected.ticker}</span></h2></div><span className={`grade ${gradeClass(selected.grade)} large`}>{selected.grade}</span></div>{selected.valuation ? <div className="price-band"><div><span>收盤價</span><strong>{selected.valuation.closingPrice === null ? "資料待補" : `${selected.valuation.closingPrice.toFixed(2)} 元`}</strong></div><div><span>市值</span><strong>{formatTwd(selected.valuation.marketCapTwd)}</strong></div><div><span>台股市值排名</span><strong>{selected.valuation.marketCapRank === null ? "資料待補" : `#${selected.valuation.marketCapRank}`}</strong></div><div><span>PE</span><strong>{formatMultiple(selected.valuation.peRatio)}</strong></div><div><span>P/B</span><strong>{formatMultiple(selected.valuation.pbRatio)}</strong></div><div><span>現金殖利率</span><strong>{formatPercent(selected.valuation.dividendYield)}</strong></div></div> : null}<section className="detail-scores"><div className="explanation"><strong>{selected.evaluatedCount === 11 ? "11 項評分完成" : `目前已完成 ${selected.evaluatedCount} / 11 項可量化評估`}</strong><p>個股數值以最新市場快照呈現；同業基準採相同產業上市公司中位數。未具備經驗證資料的項目保留「資料待補」，不納入 A/B/C 分級。</p></div><div className="factor-grid">{selected.factors.map((factor) => <FactorCard factor={factor} key={factor.id} />)}</div></section><section className="recorded-data" aria-label="各項評比已記錄數據"><div className="section-heading"><div><span className="eyebrow">評比資料明細</span><h2>各項評比數據變化</h2></div></div><p>依各項評比顯示歷次快照；尚無歷史紀錄時，保留最新數值與比較基準。</p><div className="record-grid">{selected.factors.map((factor) => <section className="record-card" key={factor.id}><div><strong>{factor.name}</strong><span className={`record-state ${factor.state}`}>{factorStatus(factor)}</span></div><dl><div><dt>最新數值</dt><dd>{factor.value ?? "資料待補"}</dd></div><div><dt>比較基準</dt><dd>{factor.benchmark ?? "尚無可比較基準"}</dd></div><div><dt>資料期間</dt><dd>{factor.period ?? "尚未記錄"}</dd></div><div><dt>資料來源</dt><dd>{factor.source}</dd></div></dl>{factor.history?.length ? <ol className="history-list">{factor.history.map((point) => <li key={`${point.capturedAt}:${point.period ?? ""}`}><strong>{new Date(point.capturedAt).toLocaleDateString("zh-TW")}</strong><span>{point.value ?? "資料待補"}</span><span>{point.benchmark ?? "無比較基準"}</span><small>{point.period ?? "未記錄期間"} · {factorStatus(point)}</small></li>)}</ol> : <small className="no-history">尚無歷史紀錄。</small>}<p>{factor.note}</p>{factor.evidence?.length ? <ul>{factor.evidence.map((evidence) => <li key={`${evidence.criterion}:${evidence.sourceUrl}`}><a href={evidence.sourceUrl} target="_blank" rel="noreferrer">{evidence.criterion}</a>：{evidence.observation}</li>)}</ul> : null}</section>)}</div></section><footer className="source-note">來源：{snapshot.sources.join("、")}。數值與同業比較為研究輔助，非買賣建議。</footer></article> : null}
  </main>;
}
